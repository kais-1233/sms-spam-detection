...
class SimpleStorageService:
    ...
    def read_object(object_name: object, decode: bool = True, make_readable: bool = False) -> Union[StringIO, str]:
        """
        Read S3 object body with decode options
        """
        logging.info("Entered read_object method")

        try:
            # Ensure object_name supports .get()
            response = object_name.get()

            raw_content = response["Body"].read()
            content = raw_content.decode() if decode else raw_content
    ...
    def get_file_object(self, filename: str, bucket_name: str):
        """
        Returns first matching S3 file object
        """
        ...
        try:
            ...
            if len(file_objects) == 0:
                raise Exception(f"S3 file not found: {filename}")
    ...
    def load_model(self, model_name: str, bucket_name: str, model_dir: str = None):
        """
        Load pickle model from S3
        """
        logging.info("Entered load_model method")

        try:
            # FORCE best_model.pkl (final fix)
            model_name = "best_model.pkl"

            model_key = model_name if model_dir is None else model_dir + "/" + model_name
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "404":
                self.s3_client.put_object(
                    Bucket=bucket_name, Key=folder_name + "/"
                )
        except Exception as e:
            raise SpamhamException(e, sys)
    ...
    def upload_df_as_csv(self, data_frame: DataFrame, local_filename: str, bucket_filename: str, bucket_name: str):
        """
        Save dataframe → CSV → upload to S3
        """
    ...
    def get_df_from_object(self, object_: object) -> DataFrame:
        """
        Convert S3 CSV object → DataFrame
        """
